...
import { fetchBillingStatus } from '@/hooks/api/billing.hooks'
...
import { useCreateScanMutation, useScanHistoryQuery } from '@/hooks/api/scans.hooks'
import type { ClearbitSuggestion, EntityCandidate, HistoryRow } from '@/hooks/api/types'
import { useCreditsQuery } from '@/hooks/api/users.hooks'
import { useAuthStore } from '@/stores/authStore'
import { VerdictBadge, StatusDot } from '@/components/ui/Badge'
...
function quickPicks(rows: HistoryRow[]): HistoryRow[] {
  const seen = new Set<string>()
  const out: HistoryRow[] = []
  for (const r of rows) {
    const k = (r.domain || r.entity_name || '').toLowerCase()
...
function laneDots(coverage: number) {
  const filled = Math.min(3, coverage >= 4 ? 3 : Math.max(0, coverage))
  return Array.from({ length: 3 }, (_, i) => (
    <StatusDot key={i} status={i < filled ? 'complete' : 'queued'} />
  ))
}

/** If the user typed a domain, send domain_hint so the API can return a hint candidate (see resolver). */
...
export function Dashboard() {
  ...
  const historyQuery = useScanHistoryQuery(enabled, 20)
  ...
  const picks = useMemo(() => quickPicks(history), [history])

  const busy =
    resolveMut.isPending || confirmMut.isPending || createScanMut.isPending

  useEffect(() => {
    document.title = 'Dashboard — DealScannr'
  }, [])
  ...
  const showUpgradeToast = searchParams.get('upgraded') === 'true'
  useEffect(() => {
    if (!token || !showUpgradeToast) return
    let cancelled = false
    ;(async () => {
      try {
        ...
        if (p === 'pro') setUpgradeToast('Welcome to Pro — 50 scans/month unlocked')
        else if (p === 'team') setUpgradeToast('Welcome to Team — 200 scans/month unlocked')
        else setUpgradeToast('Subscription updated — thank you.')
      } catch {
        if (!cancelled) setUpgradeToast('Subscription updated — thank you.')
      }
      const next = new URLSearchParams(searchParams)
      next.delete('upgraded')
      setSearchParams(next, { replace: true })
      window.setTimeout(() => setUpgradeToast(null), 6500)
    })()
    return () => {
      cancelled = true
    }
  }, [token, showUpgradeToast, searchParams, setSearchParams])
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void (async () => {
        try {
          const res = await axiosInstance.get<ClearbitSuggestion[]>(ENTITY.AUTOCOMPLETE(q), {
            signal: ac.signal,
          })
  ...
  const confirmAndStart = useCallback(async (c: EntityCandidate | null, manual?: string) => {
    setErr(null)
    ...
    try {
      ...
      if (st === 402) {
        ...
        toast('error', 'No scan credits left this month.')
      } else if (st === 429) {
        setErr('Too many scans — wait about 30 seconds and try again.')
  ...
  return (
    <div className="text-[var(--text)]">
      {upgradeToast && (
        <div className="fixed bottom-24 left-1/2 z-50 max-w-md -translate-x-1/2 rounded-[var(--radius-md)] border border-[var(--green)]/30 bg-[var(--surface)] px-4 py-3 text-center text-sm shadow-dsMd lg:bottom-6">
          {upgradeToast}
        </div>
      )}
        ...
        actions={
          credits ? (
            <div className="flex flex-wrap items-center gap-3">
              <span className="rounded-full border border-[var(--accentBorder)] bg-[var(--accentSoft)] px-2.5 py-1 text-xs font-semibold capitalize text-[var(--accent)]">
                {credits.plan}
              </span>
              <span className="text-sm text-[var(--textMuted)]">
                <span className="font-medium text-[var(--text)]">{credits.remaining}</span> scans left
              </span>
            </div>
          ) : null
        }
      ...
      <Card
        padding="lg"
        className={`max-w-3xl transition-shadow ${
      ...
      >
        ...
        >
          <div
            ref={scanInputWrapRef}
            className="scan-input-wrapper relative min-w-0 flex-1"
          >
            <div
            ...
            >
              <input
                ...
                aria-controls="scan-autocomplete-list"
                autoComplete="off"
                role="combobox"
              />
              {query ? (
        ...
        {picks.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-2">
            <span className="w-full text-xs font-medium text-[var(--textSubtle)] sm:w-auto sm:py-1">
              Recent
            </span>
            {picks.map((r) => (
              <button
                key={r.scan_id}
                type="button"
                className="rounded-full border border-[var(--border)] bg-[var(--surface2)] px-3 py-1 font-mono text-xs text-[var(--text)] hover:border-[var(--accent)] hover:bg-[var(--accentSoft)]"
                onClick={() => setQuery(r.domain || r.entity_name)}
              >
                {r.domain || r.entity_name}{' '}
                <span className="text-[var(--textMuted)]" aria-hidden>
                  ↗
                </span>
              </button>
            ))}
      ...
      {disambiguationOpen && (
        <Card padding="md" className="mt-6 max-w-3xl border-[var(--accentBorder)] bg-[var(--accentSoft)]/40">
          ...
          {pick === 'manual' && (
            <div className="mt-3">
              <label htmlFor="manual-domain" className="text-xs font-medium text-[var(--textMuted)]">
                Domain
              </label>
              <input
                id="manual-domain"
      ...
      <section id="recent-scans" className="mt-12">
        <div className="mb-4 flex items-center justify-between gap-4">
          <h2 className="font-display text-lg font-semibold text-[var(--text)]">Recent scans</h2>
          <span className="text-sm text-[var(--textSubtle)]">Last 20</span>
        </div>
        ...
        {!historyQuery.isLoading && history.length === 0 && (
          <Card padding="md">
            <EmptyState
              icon={Search}
              title="No scans yet"
              description="Enter a company name above to run your first scan."
            />
          </Card>
        )}
        ...
        {!historyQuery.isLoading && history.length > 0 && (
          <ul className="space-y-3">
            {history.map((row) => (
              <li key={row.scan_id}>
                <Link
                  to={row.has_report ? `/scan/${row.scan_id}/report` : `/scan/${row.scan_id}/progress`}
                  className="block"
                >
                  <Card hover padding="md" className="flex flex-wrap items-center gap-4">
                    <div className="min-w-0 flex-1">
                      <p className="font-medium text-[var(--text)]">
                        {row.entity_name || row.domain || 'Scan'}
                      </p>
                      <p className="font-mono text-xs text-[var(--textMuted)]">{row.domain || '—'}</p>
                    </div>
                    <div className="flex items-center gap-1.5" aria-label="Lane coverage">
                      {laneDots(row.lane_coverage)}
                    </div>
                    <div className="flex items-center gap-4">
                      {row.verdict ? (
                        <VerdictBadge verdict={row.verdict} size="lg" pulse />
                      ) : (
                        <span className="font-mono text-xs text-[var(--textSubtle)]">…</span>
                      )}
                      <div className="text-right">
                        ...
                        <ChevronRight className="ml-auto h-5 w-5 text-[var(--textSubtle)]" aria-hidden />
                      </div>
                    </div>
                  </Card>
                </Link>
